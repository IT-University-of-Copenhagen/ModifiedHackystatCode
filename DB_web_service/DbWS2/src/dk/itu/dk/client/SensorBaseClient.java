...
public class SensorBaseClient {
	...
	private Preference<MediaType> xmlMedia = new Preference<MediaType>(
			MediaType.TEXT_XML);
	/** For PMD. */
	...
	/**
	 * Timestamp of last time we tried to contact a server and failed, since
	 * this is expensive.
	 */
	...
	// JAXBContexts are thread safe, so we can share them across all instances
	// and threads.
	...
	static {
		try {
			...
			throw new RuntimeException(
					"Couldn't create JAXB context instances.", e);
		}
	}

	/**
	 * Initializes a new SensorBaseClient, given the host, userEmail, and
	 * password.
	 * 
	 * @param host
	 *            The host, such as 'http://localhost:9876/sensorbase'.
	 * @param email
	 *            The user's email that we will use for authentication.
	 * @param password
	 *            The password we will use for authentication.
	 */
	public SensorBaseClient(String host, String email, String password) {
		validateArg(host);
		validateArg(email);
		validateArg(password);
		// Restlet logger leaves too many .lck files around, so disable it until
		// we figure it out.
		...
		if (this.isTraceEnabled) {
			System.out.println("SensorBaseClient Tracing: INITIALIZE "
					+ "host='" + host + "', email='" + email + "', password='"
					+ password + "'");
		}
	 ...
	 */
	public synchronized SensorBaseClient authenticate()
			throws SensorBaseClientException {
		String uri = "ping?user=" + this.userEmail + "&password="
				+ this.password;
	...
	/**
	 * Provides an easy way to construct SensorData instances. The keyValMap is
	 * processed and the key-value pairs are processed in the following way.
	 * <ul>
	 * <li>["Owner", email] (if not supplied, defaults to this client's email)
	 * <li>["Timestamp", timestamp string] (if not supplied, defaults to the
	 * current time.)
	 * <li>["Runtime", timestamp string] (if not supplied, defaults to
	 * Timestamp.)
	 ...
	 * <li>Any other key-val pairs are extracted and put into the SensorData
	 * properties.
	 * </ul>
	 * Throws an exception if Timestamp or Runtime are supplied but can't be
	 * parsed into an XMLGregorianCalendar instance.
	 * 
	 * @param keyValMap
	 *            The map of key-value pairs corresponding to SensorData fields
	 *            and properties.
	 * @return A SensorData instance.
	 * @throws SensorBaseClientException
	 *             If errors occur parsing the contents of the keyValMap.
	 */
	public synchronized SensorData makeSensorData(Map<String, String> keyValMap)
		...
		try {
			tstamp = Tstamp.makeTimestamp(takeFromMap(keyValMap, "Timestamp",
					defaultTstamp.toString()));
		} catch (Exception e) {
			throw new SensorBaseClientException("Error parsing tstamp", e);
		}
		XMLGregorianCalendar runtime = null;
		try {
			runtime = Tstamp.makeTimestamp(takeFromMap(keyValMap, "Runtime",
					defaultTstamp.toString()));
		} catch (Exception e) {
		...
		data
				.setProperties(new org.hackystat.sensorbase.resource.sensordata.jaxb.Properties());
	...
	/**
	 * Returns the value associated with key in keyValMap, or the default, and
	 * also removes the mapping associated with key from the keyValMap.
	 * 
	 * @param keyValMap
	 *            The map
	 * @param key
	 ...
	 */
	private String takeFromMap(Map<String, String> keyValMap, String key,
			String defaultValue) {
		String value = (keyValMap.get(key) == null) ? defaultValue : keyValMap
				.get(key);
	 ...
	 *             that cannot be marshalled into Java SensorDataTypeIndex
	 *             instance.
	 */
	/*
...
*/
	...
	public synchronized void putSensorDataType(SensorDataType sdt)
		...
		try {
			...
			Representation representation = SensorBaseResource
					.getStringRepresentation(xmlData);
			String uri = "sensordatatypes/" + sdt.getName();
			// String uri = "/storeSensorDataType/" + sdt.getName();
	 ...
	 * @throws SensorBaseClientException
	 *             If the server does not indicate success.
	 */
/*	
	public synchronized void deleteSensorDataType(String sdtName)
			throws SensorBaseClientException {
		Response response = makeRequest(Method.DELETE, "sensordatatypes/"
				+ sdtName, null);
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
...
*/
	/**
	 * Returns the index of Users from this server. This is an admin-only
	 * operation.
	 * 
	 ...
	 */
/*
	public synchronized UserIndex getUserIndex()
			throws SensorBaseClientException {
...
*/
	 ...
	 * 
	 * @return The User instance for the user associated with this
	 *         SensorBaseClient.
	 * @throws SensorBaseClientException
	 *             If problems occur.
	 */
	...
	public synchronized User getUser(String email)
			throws SensorBaseClientException {
		/*
		 * Bugfix Martin Imme: use email rather than userEmail in order to
		 * retrieve the correct user data
		 */
		// Response response = makeRequest(Method.GET, "users/" + email, null);
		Response response = makeRequest(Method.GET, "getUser/" + email, null);
		User user;
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
	 ...
	 *             that cannot be marshalled into Java User instance.
	 */
	public synchronized User getUser(UserRef ref)
			throws SensorBaseClientException {
		Response response = getUri(ref.getHref());
		User user;
		if (!response.getStatus().isSuccess()) {
	...
	public synchronized void deleteUser(String email)
			throws SensorBaseClientException {
		// Response response = makeRequest(Method.DELETE, "users/" + email,
		// null);
		Response response = makeRequest(Method.DELETE, "deleteUser/" + email,
				null);
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
	 ...
	 * 
	 * @param email
	 *            The email of the User whose properties are to be deleted.
	 * @param properties
	 *            The properties to post.
	 * @throws SensorBaseClientException
	 *             If the server does not indicate success.
	 */
	public synchronized void updateUserProperties(String email,
			Properties properties) throws SensorBaseClientException {
		String xmlData;
		try {
			xmlData = makeProperties(properties);
		} catch (Exception e) {
			throw new SensorBaseClientException(
					"Failed to marshall Properties instance.", e);
		}
		Representation representation = SensorBaseResource
				.getStringRepresentation(xmlData);
		Response response = makeRequest(Method.POST, "users/" + email,
				representation);
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
	...
	/**
	 * GETs the URI string and returns the Restlet Response if the server
	 * indicates success.
	 * 
	 * @param uriString
	 *            The URI String, such as
	 *            "http://localhost:9876/sensorbase/sensordatatypes".
	 * @return The response instance if the GET request succeeded.
	 * @throws SensorBaseClientException
	 *             If the server indicates that a problem occurred.
	 */
	public synchronized Response getUri(String uriString)
			throws SensorBaseClientException {
	...
	/**
	 * Returns the index of SensorData from this server. This is an admin-only
	 * operation.
	 * 
	 ...
	 */
/*	
	public synchronized SensorDataIndex getSensorDataIndex()
			throws SensorBaseClientException {
...
*/
	/**
	 * Computes the lastMod value for this index. Iterates through the
	 * individual refs to find their lastMod values, and stores the most recent
	 * lastMod value as the result. If the index is empty, then a default
	 * lastMod of 1000-01-01 is returned. We hope that indexes are cached so
	 * that this is not done a lot.
	 * 
	 * @param index
	 *            The index.
	 * @throws SensorBaseClientException
	 *             If problems occur parsing the lastMod fields.
	 */
	private void setSensorDataIndexLastMod(SensorDataIndex index)
			throws SensorBaseClientException {
		try {
			...
			for (SensorDataRef ref : index.getSensorDataRef()) {
				...
				if ((!(lastMod == null))
						&& Tstamp.greaterThan(lastMod, index.getLastMod())) {
					index.setLastMod(lastMod);
				}
			}
		} catch (Exception e) {
			throw new SensorBaseClientException(
					"Error setting LastMod for index: " + index, e);
		}
	}
	...
	/**
	 * Returns the index of SensorData for this user from this server with the
	 * specified SDT.
	 * 
	 * @param email
	 *            The user email.
...
/*	
	public synchronized SensorDataIndex getSensorDataIndex(String email,
			String sdtName) throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, sensordataUri + email
				+ "?sdt=" + sdtName, null);
		SensorDataIndex index;
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
...
*/
	/**
	 * Returns an index to SensorData for the specified user of all sensor data
	 * that have arrived at the server since the specified start and end times.
	 * Uses the LastMod field to determine what data will be retrieved.
	 * <p>
	 * Note that data could be sent recently with a Timestamp (as opposed to
	 * LastMod field) from far back in the past, and the index will include
	 * references to such data. This method thus differs from all other
	 * SensorDataIndex-returning methods, because the others compare passed
	 * timestamp values to the Timestamp associated with the moment at which a
	 * sensor data instance is created, not the moment it ends up being received
	 * by the server.
	 * <p>
	 * This method is intended for use by user interface facilities such as the
	 * SensorDataViewer that wish to monitor the arrival of data at the
	 * SensorBase.
	 * 
	 * @param email
	 *            The user email.
	 * @param lastModStartTime
	 *            A timestamp used to determine the start time of data to get.
	 * @param lastModEndTime
	...
	public synchronized SensorDataIndex getSensorDataIndexLastMod(String email,
			...
			throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, sensordataUri + email
				+ "?lastModStartTime=" + lastModStartTime + "&lastModEndTime="
				+ lastModEndTime, null);
		SensorDataIndex index;
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
	...
	/**
	 * Returns the SensorData for this user from this server with the specified
	 * timestamp. Uses the cache if enabled.
	 * 
	 * @param email
	 *            The user email.
	 * @param timestamp
	 ...
	 *             String that cannot be marshalled into Java SensorData
	 *             instance.
	 */
/*	
	public synchronized SensorData getSensorData(String email,
			XMLGregorianCalendar timestamp) throws SensorBaseClientException {
		SensorData data;
		String uri = sensordataUri + email + "/" + timestamp;
		// Check the cache, and return the sensor data instance from it if
		// available.
...
*/
	/**
	 * Returns the SensorData for this user from this server given the passed
	 * uriString suffix.
	 * 
	 * @param uriString
	 *            A string that when prefixed with the sensorbase host should
	 *            return a SensorData instance in XML format. A string such as
	 *            "http://localhost:9876/sensorbase" will be prefixed to the
	 *            uriString.
	 * @return The SensorData instance.
	 * @throws SensorBaseClientException
	 *             If the server does not return the success code or returns a
	...
	public synchronized SensorData getSensorData(String uriString)
			throws SensorBaseClientException {
		SensorData data;
		// Check the cache, and return the sensor data instance from it if
		// available.
	 ...
	 */
	public synchronized SensorData getSensorData(SensorDataRef ref)
			throws SensorBaseClientException {
		SensorData data;
	...
	public synchronized void putSensorData(SensorData data)
		...
		try {
			...
			Representation representation = SensorBaseResource
					.getStringRepresentation(xmlData);
			String uri = sensordataUri + data.getOwner() + "/"
					+ data.getTimestamp();
			Response response = makeRequest(Method.PUT, uri, representation);
			if (!response.getStatus().isSuccess()) {
				throw new SensorBaseClientException(response.getStatus());
		...
		// All other exceptions are caught and rethrown.
			...
			throw new SensorBaseClientException(
					"Error marshalling sensor data", e);
		}
	}

	/**
	 * Creates the passed batch of SensorData on the server. Assumes that all of
	 * them have the same owner field, and that batch is non-empty.
	 * 
	 * @param data
	 ...
	 *             If problems occur posting this data.
	 */
	public synchronized void putSensorDataBatch(SensorDatas data)
			throws SensorBaseClientException {
		try {
		...
		// All other exceptions are caught and rethrown.
			...
			throw new SensorBaseClientException(
					"Error marshalling batch sensor data", e);
		}
	}

	/**
	 * Ensures that the SensorData instance with the specified user and tstamp
	 * is not on the server. Returns success even if the SensorData instance did
	 ...
	 */
/*	
	public synchronized void deleteSensorData(String email,
			XMLGregorianCalendar timestamp) throws SensorBaseClientException {
		Response response = makeRequest(Method.DELETE, sensordataUri + email
				+ "/" + timestamp, null);
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
...
*/
	/**
	 * Deletes all sensor data associated with the specified user. Note that the
	 * user must be a test user. Returns success even if the user had no sensor
	 * data.
	 * 
	 * @param email
	 *            The email of the User.
	...
	/**
	 * Returns the index of all Projects from this server. This is an admin-only
	 * operation.
	 * 
	 ...
	 */
	public synchronized ProjectIndex getProjectIndex()
			throws SensorBaseClientException {
	...
	/**
	 * Returns the index of all Projects from this server associated with this
	 * user. This includes the projects that this user owns, that this user is a
	 * member of, and that this user has been invited to participate in as a
	 * member (but has not yet accepted or declined.)
	 * 
	 * @param email
	 *            The user email.
	 * @return The ProjectIndex instance.
	...
	public synchronized Project getProject(String email, String projectName)
			throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, projectsUri + email + "/"
				+ projectName, null);
		Project project;
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
	...
	/**
	 * True if the current user is the owner, member, or spectator of the
	 * specified project. If caching is enabled, then we cache the retrieved
	 * project definition for 3 minutes when this client is an owner, member, or
	 * spectator so that subsequent retrievals don't require an http access.
	 * 
	 * @param email
	...
	/**
	 * Returns true if this client's user is the owner or member or spectator in
	 * the project.
	 * 
	 * @param project
	 *            The project.
	...
	/**
	 * Invites the user indicated via their email address to the named project
	 * owned by this user. Has no effect if the user is already an invited
	 * member. Returns the updated project representation.
	 * 
	 * @param email
	 *            The user to be invited to this project.
	 * @param projectName
	 *            The project name.
	 * @return The project representation as a result of the invitation.
	 * @throws SensorBaseClientException
	 *             If the server does not return success.
	 */
	 ...
	 * 
	 * @param owner
	 *            The owner of the project that this user has been invited into
	 * @param projectName
	 *            the name of the project.
	 * @param reply
	 ...
	 */
	public synchronized void reply(String owner, String projectName,
			InvitationReply reply) throws SensorBaseClientException {
	 ...
	 * 
	 * @param owner
	 *            The owner of the project to be renamed.
	 * @param projectName
	 ...
	 */
	public synchronized void renameProject(String owner, String projectName,
			String newProjectName) throws SensorBaseClientException {
		Response response = makeRequest(Method.POST, projectsUri + owner + "/"
				+ projectName + "/rename/" + newProjectName, null);
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
	}
	 ...
	 *             that cannot be marshalled into Java Project instance.
	 */
	public synchronized Project getProject(ProjectRef ref)
			throws SensorBaseClientException {
		Response response = getUri(ref.getHref());
		Project project;
		if (!response.getStatus().isSuccess()) {
	...
	/**
	 * Returns a SensorDataIndex representing all the SensorData for this
	 * Project.
	 * 
	 * @param owner
	 ...
	 *             that cannot be marshalled into Java SensorDataIndex instance.
	 */
	public synchronized SensorDataIndex getProjectSensorData(String owner,
			String projectName) throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, projectsUri + owner + "/"
				+ projectName + "/sensordata", null);
		SensorDataIndex index;
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
	...
	/**
	 * Returns a SensorDataIndex representing the SensorData for the Project
	 ...
	 */
	public synchronized SensorDataIndex getProjectSensorData(String owner,
			String projectName, XMLGregorianCalendar startTime,
			XMLGregorianCalendar endTime) throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, projectsUri + owner + "/"
				+ projectName + "/sensordata?startTime=" + startTime
				+ andEndTime + endTime, null);
		SensorDataIndex index;
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
	...
	/**
	 * Returns a SensorDataIndex representing the SensorData with the given SDT
	 * for the Project during the time interval.
	 * 
	 * @param owner
	 *            The project owner's email.
	 * @param projectName
	...
	public synchronized SensorDataIndex getProjectSensorData(String owner,
			String projectName, XMLGregorianCalendar startTime,
			XMLGregorianCalendar endTime, String sdt)
			throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, projectsUri + owner + "/"
				+ projectName + "/sensordata?sdt=" + sdt + "&startTime="
				+ startTime + andEndTime + endTime, null);
		SensorDataIndex index;
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
	...
	public synchronized SensorDataIndex getProjectSensorData(String owner,
			String projectName, XMLGregorianCalendar startTime,
			XMLGregorianCalendar endTime, String sdt, String tool)
			throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, projectsUri + owner + "/"
				+ projectName + "/sensordata?sdt=" + sdt + "&startTime="
				+ startTime + andEndTime + endTime + "&tool=" + tool, null);
		SensorDataIndex index;
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
	...
	/**
	 * Returns a SensorDataIndex representing the SensorData with the startIndex
	 * and maxInstances for the Project during the time interval. The startIndex
	 * must be non-negative, and is zero-based. The maxInstances must be
	 * non-negative. If startIndex is greater than the number of instances in
	 * the time interval, then an empty SensorDataIndex is returned.
	 * 
	 * @param owner
	 *            The project owner's email.
	 * @param projectName
	 ...
	 *            The end time.
	 * @param startIndex
	 *            The zero-based index to the first Sensor Data instance to be
	 *            returned in the time interval, when the instances are all
	...
	public synchronized SensorDataIndex getProjectSensorData(String owner,
			String projectName, XMLGregorianCalendar startTime,
			XMLGregorianCalendar endTime, int startIndex, int maxInstances)
			throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, projectsUri + owner + "/"
				+ projectName + "/sensordata?startTime=" + startTime
				+ andEndTime + endTime + "&startIndex=" + startIndex
	...
	/**
	 * Returns a SensorDataIndex containing a snapshot of the sensor data for
	 * the given project and sdt during the specified time interval. A
	 * "snapshot" is the set of sensor data with the most recent runtime value
	 ...
	 *             If problems occur.
	 */
	public synchronized SensorDataIndex getProjectSensorDataSnapshot(
			String owner, String projectName, XMLGregorianCalendar startTime,
			XMLGregorianCalendar endTime, String sdt)
			throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, projectsUri + owner + "/"
				+ projectName + "/snapshot?startTime=" + startTime + andEndTime
				+ endTime + "&sdt=" + sdt, null);
		SensorDataIndex index;
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
	...
	/**
	 * Returns a SensorDataIndex containing a snapshot of the sensor data for
	 * the given project, sdt and tool during the specified time interval. A
	 * "snapshot" is the set of sensor data with the most recent runtime value
	 * during that time interval.
	 * 
	 * @param owner
	...
	public synchronized SensorDataIndex getProjectSensorDataSnapshot(
		...
		Response response = makeRequest(Method.GET, projectsUri + owner + "/"
				+ projectName + "/snapshot?startTime=" + startTime + andEndTime
				+ endTime + "&sdt=" + sdt + "&tool=" + tool, null);
		SensorDataIndex index;
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
	...
	/**
	 * Returns a ProjectSummary representing a summary of the number of sensor
	 * data instances of each type for the given interval.
	 * 
	 * @param owner
	 *            The project owner.
	 * @param projectName
	 ...
	 *             If problems occur.
	 */
	public synchronized ProjectSummary getProjectSummary(String owner,
			String projectName, XMLGregorianCalendar startTime,
			XMLGregorianCalendar endTime) throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, projectsUri + owner + "/"
				+ projectName + "/summary?startTime=" + startTime + andEndTime
				+ endTime, null);
	 ...
	 *            The start day.
	 * @param numDays
	 *            The number of days, each one getting a ProjectSummary
	 *            instance.
	 * @return The MulitDayProjectSummary instance.
	 * @throws SensorBaseClientException
	 *             If problems occur.
	 */
	public synchronized MultiDayProjectSummary getMultiDayProjectSummary(
			String owner, String projectName, XMLGregorianCalendar startTime,
			int numDays) throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, projectsUri + owner + "/"
				+ projectName + "/summary?startTime=" + startTime + "&numDays="
				+ numDays, null);
	...
	/**
	 * Returns a MultiDayProjectSummary for the specified project, year, and
	 * month (zero based).
	 * 
	 * @param owner
	 *            The project owner.
	 ...
	 *             If problems occur.
	 */
	public synchronized MultiDayProjectSummary getMonthProjectSummary(
			String owner, String projectName, int year, int month)
			throws SensorBaseClientException {
		XMLGregorianCalendar startDay = Tstamp.makeTimestamp();
		startDay.setDay(1);
		startDay.setMonth(month);
		startDay.setYear(year);
		int numDays = startDay.toGregorianCalendar().getActualMaximum(
	...
	public synchronized void putProject(Project project)
		...
		try {
			...
			Representation representation = SensorBaseResource
					.getStringRepresentation(xmlData);
			String uri = projectsUri + project.getOwner() + "/"
					+ project.getName();
			Response response = makeRequest(Method.PUT, uri, representation);
			if (!response.getStatus().isSuccess()) {
				throw new SensorBaseClientException(response.getStatus());
	...
	public synchronized void deleteProject(String email, String projectName)
			throws SensorBaseClientException {
		Response response = makeRequest(Method.DELETE, projectsUri + email
				+ "/" + projectName, null);
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
	...
	/**
	 * Registers the given user email with the given SensorBase. Timeout is set
	 * to 5 seconds.
	 * 
	 * @param host
	 *            The host name, such as "http://localhost:9876/sensorbase".
	 * @param email
	 *            The user email.
	 * @throws SensorBaseClientException
	 *             If problems occur during registration.
	 */
	public static void registerUser(String host, String email)
			throws SensorBaseClientException {
		RestletLoggerUtil.disableLogging();
		String registerUri = host.endsWith("/") ? host + "register" : host
				+ "/register";
	 ...
	 * 
	 * @param host
	 *            The host that was determined to be not available.
	 */
	...
	/**
	 * Gets the lastHostNotAvailable timestamp associated with host. Returns 0
	 * if there is no lastHostNotAvailable timestamp.
	 * 
	 * @param host
	 *            The host whose lastNotAvailable timestamp is to be retrieved.
	...
	/**
	 * Returns true if the passed host is a SensorBase host. The timeout is set
	 * at the default timeout value. Since checking isHost() when the host is
	 * not available is expensive, we cache the timestamp whenever we find the
	 * host to be unavailable and if there is another call to isHost() within
	 * two seconds, we will immediately return false. This makes startup of
	 * clients like SensorShell go much faster, since they call isHost() several
	 * times during startup.
	 * 
	 * @param host
	 *            The URL of a sensorbase host, such as
	...
	public static boolean isHost(String host) {
		RestletLoggerUtil.disableLogging();
		// We return false immediately if we failed to contact the host within
		...
		try {
			String registerUri = host.endsWith("/") ? host + "ping" : host
					+ "/ping";
			Request request = new Request();
			request.setResourceRef(registerUri);
			...
			boolean isAvailable = (response.getStatus().isSuccess() && "SensorBase"
					.equals(pingText));
			if (!isAvailable) {
				setLastHostNotAvailable(host);
			}
	...
	/**
	 * Returns true if the user and password is registered as a user with this
	 * host.
	 * 
	 * @param host
	 *            The URL of a sensorbase host, such as
	 ...
	 */
	public static boolean isRegistered(String host, String email,
			String password) {
		RestletLoggerUtil.disableLogging();
		...
		try {
			SensorBaseClient client = new SensorBaseClient(host, email,
					password);
			client.authenticate();
			return true;
		} catch (Exception e) {
	...
	/**
	 * Takes a String encoding of a SensorDataType in XML format and converts it
	 * to an instance.
	 * 
	 * @param xmlString
	 ...
	 */
	private SensorDataType makeSensorDataType(String xmlString)
			throws Exception {
		Unmarshaller unmarshaller = sdtJAXB.createUnmarshaller();
		return (SensorDataType) unmarshaller.unmarshal(new StringReader(
				xmlString));
	}

	/**
	 * Takes a String encoding of a SensorDataTypeIndex in XML format and
	 * converts it to an instance.
	 * 
	 * @param xmlString
	 ...
	 *             If problems occur during unmarshalling.
	 */
	private SensorDataTypeIndex makeSensorDataTypeIndex(String xmlString)
			throws Exception {
		Unmarshaller unmarshaller = sdtJAXB.createUnmarshaller();
		return (SensorDataTypeIndex) unmarshaller.unmarshal(new StringReader(
				xmlString));
	}

	/**
	 * Returns the passed SensorDataType instance as a String encoding of its
	 * XML representation.
	 * 
	 * @param sdt
	...
	/**
	 * Takes a String encoding of a User in XML format and converts it to an
	 * instance.
	 * 
	 * @param xmlString
	...
	/**
	 * Takes a String encoding of a UserIndex in XML format and converts it to
	 * an instance.
	 * 
	 * @param xmlString
	...
	/**
	 * Returns the passed Properties instance as a String encoding of its XML
	 * representation.
	 * 
	 * @param properties
	...
	/**
	 * Takes an XML Document representing a SensorDataIndex and converts it to
	 * an instance.
	 * 
	 * @param xmlString
	 ...
	 *             If problems occur during unmarshalling.
	 */
	private SensorDataIndex makeSensorDataIndex(String xmlString)
			throws Exception {
		Unmarshaller unmarshaller = sensordataJAXB.createUnmarshaller();
		return (SensorDataIndex) unmarshaller.unmarshal(new StringReader(
				xmlString));
	}

	/**
	 * Takes an XML Document representing a ProjectSummary and converts it to an
	 * instance.
	 * 
	 * @param xmlString
	 *            The XML string representing a ProjectSummary.
	 * @return The corresponding ProjectSummary instance.
	 * @throws Exception
	 *             If problems occur during unmarshalling.
	 */
	private ProjectSummary makeProjectSummary(String xmlString)
			throws Exception {
		Unmarshaller unmarshaller = projectJAXB.createUnmarshaller();
		return (ProjectSummary) unmarshaller.unmarshal(new StringReader(
				xmlString));
	}

	/**
	 * Takes an XML Document representing a MultiDayProjectSummary and converts
	 * it to an instance.
	 * 
	 * @param xmlString
	 *            The XML string representing a MultiDayProjectSummary.
	 * @return The corresponding MultiDayProjectSummary instance.
	 * @throws Exception
	 *             If problems occur during unmarshalling.
	 */
	private MultiDayProjectSummary makeMultiDayProjectSummary(String xmlString)
			throws Exception {
		Unmarshaller unmarshaller = projectJAXB.createUnmarshaller();
		return (MultiDayProjectSummary) unmarshaller
				.unmarshal(new StringReader(xmlString));
	}

	/**
	 * Takes a String encoding of a SensorData in XML format and converts it to
	 * an instance.
	 * 
	 * @param xmlString
	 ...
	 * representation. Final because it's called in constructor.
	 * 
	 * @param data
	...
	/**
	 * Returns the passed SensorDatas instance as a String encoding of its XML
	 * representation.
	 * 
	 * @param data
	 *            The SensorDatas instance.
	 * @return The XML String representation.
	 * @throws Exception
	 *             If problems occur during translation.
	 */
	...
	/**
	 * Takes a String encoding of a Project in XML format and converts it to an
	 * instance.
	 * 
	 * @param xmlString
	...
	/**
	 * Takes a String encoding of a ProjectIndex in XML format and converts it
	 * to an instance.
	 * 
	 * @param xmlString
	...
	private ProjectIndex makeProjectIndex(String xmlString) throws Exception {
		Unmarshaller unmarshaller = projectJAXB.createUnmarshaller();
		return (ProjectIndex) unmarshaller
				.unmarshal(new StringReader(xmlString));
	}

	/**
	 * Returns the passed Project instance as a String encoding of its XML
	 * representation.
	 * 
	 * @param project
	...
	private static void setClientTimeout(Client client, int milliseconds) {
		...
		// client.getContext().getParameters().add("connectTimeout",
	...
	/**
	 * Enables caching in this client. If caching has already been enabled, then
	 * does nothing.
	 * 
	 * @param cacheName
	 *            The name of the cache.
	 * @param subDir
	 *            The subdirectory in which the cache backend store is saved.
	 * @param maxLife
	 *            The default expiration time for objects, in days.
	 * @param capacity
	 *            The maximum number of instances to be held in-memory.
	 */
	public synchronized void enableCaching(String cacheName, String subDir,
			Double maxLife, Long capacity) {
		if (!this.isCacheEnabled) {
	...
	/**
	 * Delete all entries from this cache. If the cache is not enabled, then
	 * does nothing.
	 */
	...
	/**
	 * Compresses the server database tables. You must be the admin user in
	...
	public synchronized void compressTables() throws SensorBaseClientException {
		try {
			Response response = makeRequest(Method.PUT, "db/table/compress",
					null);
			if (!response.getStatus().isSuccess()) {
				throw new SensorBaseClientException(response.getStatus());
			}
	...
	/**
	 * Indexes the server database tables. You must be the admin user in order
	...
	/**
	 * Gets the rowcount for the specified table. You must be the admin user in
	 * order for this command to succeed.
	 * 
	 * @param table
	 ...
	 * @throws SensorBaseClientException
	 *             If problems occur posting this data. This can happen if the
	 *             user is not the admin user, or if the table name is invalid.
	 */
	public synchronized int rowCount(String table)
			throws SensorBaseClientException {
		try {
			Response response = makeRequest(Method.GET, "db/table/" + table
					+ "/rowcount", null);
			if (!response.getStatus().isSuccess()) {
				throw new SensorBaseClientException(response.getStatus());
			}
	...
	public synchronized String testcase(String email)
			throws SensorBaseClientException {
		String a = "";
		try {
			Response response = makeRequest(Method.GET, "test/" + email, null);
			if (!response.getStatus().isSuccess()) {
				throw new SensorBaseClientException(response.getStatus());
			}
			a = response.getEntity().getText();
		}
		// Allow SensorBaseClientExceptions to be thrown out of this method.
		catch (SensorBaseClientException f) {
			throw f;
		...
		// All other exceptions are caught and rethrown.
			...
			throw new SensorBaseClientException("test case function error", e);
		}
	...
	// just for test
	/**
	 * The following fuinction was added by Rui and Nan
	 * The function is only focus on the database access
	 * 
	 */
	public synchronized String getUserNy(String email)
			throws SensorBaseClientException {
		String xmlData = "";
		String uri = "getUser" + "/" + email;
		Response response = makeRequest(Method.GET, uri, null);
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
		try {
			xmlData = response.getEntity().getText();
		} catch (Exception e) {
			throw new SensorBaseClientException(response.getStatus(), e);
		}
		return xmlData;
	...
	public synchronized boolean storeUserNy(User user, String xmlUser,
			String xmlUserRef) throws SensorBaseClientException {
		boolean res = false;
		String uri = "storeUser" + "/" + xmlUser + "/" + xmlUserRef;
		try {
			String xmlData = makeUser(user);
			Representation representation = SensorBaseResource
					.getStringRepresentation(xmlData);
			Response response = makeRequest(Method.PUT, uri, representation);
			if (response.getStatus().isSuccess()) {
	...
	public synchronized void deleteUserNy(String email)
			throws SensorBaseClientException {
		Response response = makeRequest(Method.DELETE, "deleteUser/" + email,
				null);
		if (!response.getStatus().isSuccess()) {
		...
		System.out.println("success delection");
	}

	public synchronized String getUserIndexNy()
			throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, "getUserIndex", null);
		UserIndex index;
		String xmlData = "";
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
	...
	public synchronized String getProjectNy(User owner, String projectName)
			throws SensorBaseClientException {
		String xmlData = "";
		String ownerXml = "";
		// String email = owner.getEmail();
		try {
			ownerXml = makeUser(owner);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		String uri = "getProject" + "/" + ownerXml + "/" + projectName;
		// Representation representation =
		// SensorBaseResource.getStringRepresentation(ownerXml);
		Response response = makeRequest(Method.GET, uri, null);
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
	...
	public synchronized boolean storeProject(Project project,
			String xmlProject, String xmlProjectRef)
			throws SensorBaseClientException {
		boolean ok = false;
		String uri = "storeProject" + "/" + xmlProject + "/" + xmlProjectRef;
		try {
			String xmlData = makeProject(project);
			Representation representation = SensorBaseResource
					.getStringRepresentation(xmlData);
	...
	public synchronized void deleteProjectNy(User owner, String projectName)
			throws SensorBaseClientException {
		String email = owner.getEmail();
		String xmlData = "";
		try {
			xmlData = makeUser(owner);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		Response response = makeRequest(Method.DELETE, "deleteProject" + "/"
				+ xmlData + "/" + projectName, null);
		if (response.getStatus().isSuccess()) {
			System.out.println("success delete project " + projectName
					+ " by owner " + email);
			// throw new SensorBaseClientException(response.getStatus());
		}
		System.out.println("failure delection");
	}// end delete project

	public synchronized String getProjectIndexNy()
			throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, "getProjectIndex", null);
		ProjectIndex index;
		String xmlData = "";
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
	...
	public synchronized String getSensorDataTypeIndexNy()
			throws SensorBaseClientException {
		Response response = makeRequest(Method.GET, "getSensorDataTypeIndex",
				null);
		SensorDataTypeIndex index;
		String xmlData = "";
		if (!response.getStatus().isSuccess()) {
	...
	// Nan
	public synchronized void storeSensorData(SensorData n1, String n2, String n3)
			throws SensorBaseClientException {
		String temp = "";
		try {
			String xmlData = makeSensorData(n1);
			Representation representation = SensorBaseResource
					.getStringRepresentation(xmlData);
			// String entityString = representation.getText();
			// SensorData sensor=makeSensorData(entityString);
			// System.out.println(sensor.getTool());
			String uri = "dbmanager/" + n2 + "/" + n3;
			Response response = makeRequest(Method.PUT, uri, representation);
			if (!response.getStatus().isSuccess()) {
				throw new SensorBaseClientException(response.getStatus());
	...
	// Nan
	public synchronized String getSensorData(User user,
			XMLGregorianCalendar timestamp) throws SensorBaseClientException {

		String data;
		String uri = "dbmanager" + "/" + timestamp;
		String xmlUser;
		Representation representation = null;

		try {
			xmlUser = makeUser(user);
			representation = SensorBaseResource
					.getStringRepresentation(xmlUser);
		} catch (Exception e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		// If not in the cache, request it from the sensorbase service.
		Response response = makeRequest(Method.POST, uri, representation);
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
		try {
			data = response.getEntity().getText();
	...
	/* Nan */
	public synchronized void deleteSensorData(User user,
			XMLGregorianCalendar timestamp) throws SensorBaseClientException {

		Representation representation = null;
		String uri = "deleteSensorData" + "/" + timestamp;
		String xmlUser;
		try {
			xmlUser = makeUser(user);
			representation = SensorBaseResource
	...
	/* Nan */
	public synchronized boolean storeSensorDataType(SensorDataType sdt,
		...
		try {
			...
			String uri = "storeSensorDataType/" + xmlSensorDataType + "/"
	...
	/* Nan */
	public synchronized String getSensorDataTypeNy(String sdtName)
			throws SensorBaseClientException {
		Response response = makeRequest(Method.GET,
				"sensorDataType/" + sdtName, null);
		String sdt;
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
		try {
			sdt = response.getEntity().getText();
	...
	/* Nan */
	public synchronized void deleteSensorDataTypeNy(String sdtName)
			throws SensorBaseClientException {
		Response response = makeRequest(Method.DELETE, "deleteSensorDataType/"
				+ sdtName, null);
		if (!response.getStatus().isSuccess()) {
			throw new SensorBaseClientException(response.getStatus());
		}
